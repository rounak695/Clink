...
### Frontend: The Native Edge (`@Clink_Frontend`)
The user interface is the only part of the system the student actually experiences. It must be blazing fast, intuitive, and feel naturally integrated with their Android device.

* **React Native (Expo):** Unlike traditional frameworks that force separate codebases, React Native allows us to build a fully native Android application using a single JavaScript/TypeScript codebase. Expo provides the tooling to rapidly prototype and interact with device APIs. **User Benefit:** Fluid, 60fps native animations and immediate access to device features (like the camera for the Marketplace), ensuring students get a premium app experience.
* **NativeWind & React Native Paper:** We utilize NativeWind (Tailwind for React Native) to rapidly build UI, alongside Paper for pre-built Material Design components. **User Benefit:** The application feels premium and native to Android from day one, establishing critical user trust without requiring weeks of custom component design.
* **EAS Build & APK Distribution:** Our application is distributed natively to the device. **User Benefit:** Once installed, the application shell is already on the student's phone, requiring minimal data transfer during use compared to a web application.
...
When a student opens the Clink Android app, the native shell loads immediately. Secure REST API calls are dispatched to the Express server to fetch the campus feed, while a persistent bidirectional WebSocket is opened simultaneously to listen for live events (like immediate chat messages or post likes).
...
We prioritize the Core Loop. If the interface is not intuitive and the fundamental action of posting to a feed is broken, backend scalability does not matter. We dedicate our initial sprint entirely to UI perfection with React Native and basic database hookups.
